...
//添加跳转
const add = () => {
  router.push({name:'品牌表单',query: {title:'新增品牌',type:'add'}})
}
...
//筛选提交
const submit =  () => {
  axios.get("/select/getBrandByCondition", {
    params: {
      brandName : form.name,
      brandCode : form.code,
      status : form.status
    ...
    if (res.data.code === 1){
      ...
      for (let i = 0; i < res.data.brand.length; i++) {
        tableData.value[i] = {
          index : i+1,
          id : res.data.brand[i].BRAND_ID,
          code : res.data.brand[i].BRAND_CODE,
          name : res.data.brand[i].BRAND_NAME,
          status : res.data.brand[i].STATUS === 1?"正常":"停用",
          enable : res.data.brand[i].STATUS === 1
        }
      }
    }else {
...
//获取列表数据
//获取数据
axios.get('/select/getBrand',{params:{userId:userId}})
    .then((res) => {
      // 执行操作
      for (let i = 0; i < res.data.brand.length; i++) {
        tableData.value[i] = {
...
const changeState = ( row: any) => {
  ...
  axios.post('/update/updateBrandStatus',{
    classId : tableData.value[row.index-1].id,
  ...
  <el-container>
    <el-header style="height: 30px">
      <div style="display: flex; justify-content: space-between ;margin-top: 10px">
        <!--  标题  -->
        <el-text style="font-size: 20px "><el-icon color="#303133"><Management /></el-icon>品牌管理</el-text>
        <!--  按钮  -->
        <div>
      ...
      <div v-show="divFilter" >
        <el-form :inline="true" ref="formRef" :model="form" class="demo-form-inline">
          <el-form-item label="编号：" prop="code">
            <el-input v-model="form.code" placeholder="Brand type code" clearable />
          </el-form-item>
          <el-form-item label="名称：" prop="name">
            <el-input v-model="form.name" placeholder="Brand type name" clearable />
          </el-form-item>
