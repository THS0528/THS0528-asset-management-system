...
//添加跳转
const add = () => {
  router.push({name:'报废方式表单',query: {title:'新增报废方式',type:'add'}})
}
...
//筛选提交
const submit =  () => {
  axios.get("/select/getScrapModeByCondition", {
    params: {
      brandName : form.name,
      brandCode : form.code,
      status : form.status
    ...
    if (res.data.code === 1){
      ...
      tableData.value = []
      for (let i = 0; i < res.data.scrapMode.length; i++) {
        tableData.value[i] = {
          index : i+1,
          id : res.data.scrapMode[i].SCRAP_ID,
          code : res.data.scrapMode[i].SCRAP_CODE,
          name : res.data.scrapMode[i].SCRAP_NAME,
          status : res.data.scrapMode[i].STATUS === 1?"正常":"停用",
          enable : res.data.scrapMode[i].STATUS === 1
        }
      }
    }else {
...
const changeState = ( row: any) => {
  ...
  axios.post('/update/updateScrapModeStatus',{
    scrapModeId : tableData.value[row.index-1].id,
  ...
  <el-container>
    <el-header style="height: 30px">
      <div style="display: flex; justify-content: space-between ;margin-top: 10px">
        <!--  标题  -->
        <el-text style="font-size: 20px "><el-icon color="#303133"><Management /></el-icon>报废方式管理</el-text>
        <!--  按钮  -->
        <div>
      ...
      <div v-show="divFilter" >
        <el-form :inline="true" ref="formRef" :model="form" class="demo-form-inline">
          <el-form-item label="编号：" prop="code">
            <el-input v-model="form.code" placeholder="Scrap mode type code" clearable />
          </el-form-item>
          <el-form-item label="名称：" prop="name">
            <el-input v-model="form.name" placeholder="Scrap mode type name" clearable />
          </el-form-item>
