...
//添加跳转
const add = () => {
  router.push({name:'资产类别管理表单',query: {title:'新增资产类别',type:'add'}})
}
...
//提交
const submit =  () => {
    axios.get("/select/getAssetClassByCondition", {
          params: {
            className : form.name,
            classCode : form.code,
            status : form.status
    ...
    if (res.data.code === 1){
      ...
      tableData.value = []
      for (let i = 0; i < res.data.assetClass.length; i++) {
        tableData.value[i] = {
          index : i+1,
          id : res.data.assetClass[i].CLASS_ID,
          code : res.data.assetClass[i].CLASS_CODE,
          name : res.data.assetClass[i].CLASS_NAME,
          status : res.data.assetClass[i].STATUS === 1?"正常":"停用",
          enable : res.data.assetClass[i].STATUS === 1
        }
     }
    }else {
...
//获取列表数据
//获取数据
axios.get('/select/getAssetClass',{params:{userId:userId}})
    .then((res) => {
      // 执行操作
      for (let i = 0; i < res.data.assetClass.length; i++) {
        tableData.value[i] = {
...
const changeState = ( row: any) => {
  ...
  axios.post('/update/updateAssetClassStatus',{
